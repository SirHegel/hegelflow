...
export function SprintManager({ sprints, tasks, members, context }: {
  sprints: Sprint[];
  tasks: TaskCard[];
  members: Person[];
  ...
  return (
    <div className="mx-auto max-w-[1380px] space-y-7 px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
      ...
      {active ? (
        <ActiveSprint sprint={active} tasks={tasks.filter((task) => task.sprintId === active.id)} members={members} canManage={canManage} pending={pendingId === active.id} onComplete={() => transition(active, "complete")} />
      ) : (
      ...
      <section>
        <div className="mb-4 flex items-end justify-between"><div><h2 className="text-base font-bold text-slate-900">Próximos sprints</h2><p className="mt-1 text-xs text-slate-400">Preparados para planificación y refinamiento</p></div><Badge>{planned.length} planeados</Badge></div>
          ...
          {planned.map((sprint) => {
            ...
            return (
              <article key={sprint.id} className="surface p-5 sm:p-6">
                ...
                <div className="mt-5 grid grid-cols-3 gap-3 rounded-xl bg-slate-50 p-3 text-center"><div><p className="text-lg font-bold text-slate-900">{sprintTasks.length}</p><p className="text-[9px] uppercase tracking-wide text-slate-400">Tareas</p></div><div className="border-x border-slate-200"><p className="text-lg font-bold text-slate-900">{points}</p><p className="text-[9px] uppercase tracking-wide text-slate-400">Puntos</p></div><div><p className="text-lg font-bold text-slate-900">{new Set(sprintTasks.flatMap((task) => task.assignees.map((person) => person.id))).size}</p><p className="text-[9px] uppercase tracking-wide text-slate-400">Personas</p></div></div>
                <div className="mt-5 flex items-center justify-between"><span className="inline-flex items-center gap-1.5 text-[11px] text-slate-400"><CalendarDays className="size-3.5" /> {formatDate(sprint.startDate)} — {formatDate(sprint.endDate)}</span>{canManage ? <Button size="sm" onClick={() => transition(sprint, "start")} disabled={Boolean(pendingId) || Boolean(active)}>{pendingId === sprint.id ? <LoaderCircle className="size-4 animate-spin" /> : <Play className="size-4" />} Iniciar</Button> : null}</div>
              </article>
            );
          })}
      ...
      <section className="surface overflow-hidden">
        ...
        <div className="divide-y divide-slate-100">
          {completed.map((sprint) => {
            ...
            return <div key={sprint.id} className="flex flex-col gap-3 px-5 py-4 sm:flex-row sm:items-center sm:px-6"><CheckCircle2 className="size-5 shrink-0 text-emerald-500" /><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-slate-800">{sprint.name}</p><p className="mt-0.5 truncate text-[11px] text-slate-400">{sprint.goal || "Sin objetivo"}</p></div><div className="w-full sm:w-40"><div className="flex justify-between text-[10px] text-slate-400"><span>Completado</span><strong className="text-slate-600">{done}/{total} pts</strong></div><Progress value={percent(done, total)} className="mt-1.5" barClassName="bg-emerald-500" /></div><span className="text-[10px] text-slate-400">{formatDate(sprint.completedAt)}</span></div>;
          })}
...
function ActiveSprint({ sprint, tasks, members, canManage, pending, onComplete }: {
  sprint: Sprint;
  tasks: TaskCard[];
  members: Person[];
  canManage: boolean;
  ...
  const overdue = tasks.filter((task) => task.dueDate && !task.completedAt && new Date(`${task.dueDate}T23:59:59`) < new Date()).length;
  return (
    <section className="relative overflow-hidden rounded-2xl bg-[#202943] p-6 text-white shadow-xl shadow-slate-900/10 sm:p-8">
      ...
      <div className="relative">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between"><div><Badge color="#c4b5fd" className="bg-white/10">En ejecución</Badge><h2 className="mt-4 text-2xl font-bold sm:text-3xl">{sprint.name}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">{sprint.goal || "Sin objetivo definido."}</p></div>{canManage ? <Button variant="secondary" onClick={onComplete} disabled={pending} className="border-white/10 bg-white/10 text-white hover:bg-white/15">{pending ? <LoaderCircle className="size-4 animate-spin" /> : <CheckCircle2 className="size-4" />} Completar sprint</Button> : null}</div>
        ...
        <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex -space-x-2">{members.map((person) => <Avatar key={person.id} name={person.fullName} color={person.avatarColor} size="sm" className="ring-[#202943]" />)}</div><p className="text-xs text-slate-400">El burndown y la velocidad se actualizan desde el historial de cada transición.</p></div>
      </div>
    </section>
  );
}
