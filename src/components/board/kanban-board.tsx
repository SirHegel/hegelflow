...
export function KanbanBoard({ initialData, context }: { initialData: BoardData; context: WorkspaceContext }) {
    ...
    <div className="flex min-h-[calc(100vh-4.5rem)] flex-col">
        ...
        <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2.5">
              <span className="size-3 rounded-sm" style={{ backgroundColor: initialData.board.color }} />
              <h1 className="truncate text-xl font-bold tracking-tight text-slate-950 sm:text-2xl">{initialData.board.name}</h1>
            ...
            <p className="mt-1.5 max-w-3xl truncate text-xs text-slate-400">{initialData.board.description}</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex -space-x-1.5 pr-1">
              ...
              {initialData.members.length > 4 ? <span className="grid size-8 place-items-center rounded-full bg-slate-100 text-[10px] font-bold text-slate-500 ring-2 ring-white">+{initialData.members.length - 4}</span> : null}
            </div>
      ...
      {view === "board" ? (
        <DndContext sensors={sensors} collisionDetection={closestCorners} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
          ...
          <DragOverlay>{activeTask ? <KanbanTaskCard task={activeTask} overlay /> : null}</DragOverlay>
        </DndContext>
      ) : (
      ...
      {toast ? (
        <div className={cn("fixed bottom-5 right-5 z-[80] flex max-w-sm items-center gap-2.5 rounded-xl border bg-white px-4 py-3 text-sm font-medium shadow-xl", toast.type === "error" ? "border-rose-200 text-rose-700" : "border-emerald-200 text-emerald-700")} role="status">
          ...
          {toast.message}
          <button onClick={() => setToast(null)} className="ml-2 rounded p-0.5 opacity-60 hover:opacity-100"><X className="size-3.5" /></button>
        </div>
      ) : null}
    </div>
  );
...
function KanbanColumn({ column, tasks, canEdit, onOpen, onCreate }: {
  column: BoardColumn;
  tasks: TaskCard[];
  canEdit: boolean;
  ...
  return (
    <section className="flex w-[300px] shrink-0 flex-col" aria-label={`${column.name}, ${tasks.length} tareas`}>
      <div className="mb-3 flex items-center gap-2 px-1">
        ...
        <h2 className="text-xs font-bold uppercase tracking-wide text-slate-700">{column.name}</h2>
        <span className="grid min-w-5 place-items-center rounded-full bg-slate-200/80 px-1.5 py-0.5 text-[10px] font-bold text-slate-500">{tasks.length}</span>
        {column.wipLimit ? <span className={cn("ml-auto text-[10px] font-semibold", full ? "text-rose-600" : "text-slate-400")}>WIP {tasks.length}/{column.wipLimit}</span> : null}
        <button className={cn("rounded-lg p-1 text-slate-400 hover:bg-slate-200/60 hover:text-slate-700", !column.wipLimit && "ml-auto")} aria-label={`Opciones de ${column.name}`}><MoreHorizontal className="size-4" /></button>
      </div>
      <div
        ref={setNodeRef}
        className={cn(
      ...
      >
        <SortableContext items={tasks.map((task) => task.id)} strategy={verticalListSortingStrategy}>
          {tasks.map((task) => <KanbanTaskCard key={task.id} task={task} onOpen={onOpen} disabled={!canEdit} />)}
        </SortableContext>
        {!tasks.length ? (
            ...
            {tasks.map((task) => {
              ...
              return (
                <tr key={task.id} onClick={() => onOpen(task)} className="cursor-pointer text-xs text-slate-600 transition hover:bg-violet-50/35">
                  ...
                  <td className="max-w-sm px-4 py-4"><p className="truncate font-semibold text-slate-800">{task.title}</p><p className="mt-1 truncate text-[10px] text-slate-400">{task.taskType}</p></td>
