...
export function SettingsView({ data, context, sessionExpiresAt, timeZone }: { data: SettingsData; context: WorkspaceContext; sessionExpiresAt: string; timeZone: string }) {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-7 px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
      ...
      <section className="surface overflow-hidden">
        <SettingsHeader icon={Bot} title="Automatizaciones" description="Reglas trigger → acción sin cuotas por plan" action={canManage ? <Button size="sm" variant="secondary"><Plus className="size-4" /> Nueva regla</Button> : undefined} stackActionOnMobile />
        <div className="divide-y divide-slate-100">
          {data.rules.map((rule) => <div key={rule.id} className="flex flex-col gap-3 px-5 py-4 sm:flex-row sm:items-center sm:px-6"><div className="grid size-10 shrink-0 place-items-center rounded-xl bg-violet-50 text-violet-600"><Workflow className="size-4" /></div><div className="min-w-0 flex-1"><div className="flex min-w-0 flex-wrap items-center gap-2"><p className="min-w-0 break-words text-sm font-bold text-slate-800">{rule.name}</p><Badge color={rule.isEnabled ? "#0f9f7a" : undefined}>{rule.isEnabled ? "Activa" : "Pausada"}</Badge></div><p className="mt-1 break-words text-[11px] leading-5 text-slate-400">{humanize(rule.triggerType)} → {humanize(rule.actionType)} · {rule.runCount} ejecuciones</p></div>{canManage ? <button onClick={() => toggleRule(rule.id, !rule.isEnabled)} disabled={pendingRule === rule.id} className="inline-flex shrink-0 items-center gap-2 self-start rounded-lg px-2 py-1.5 text-xs font-semibold text-slate-500 hover:bg-slate-50 disabled:opacity-50 sm:self-auto">{pendingRule === rule.id ? <LoaderCircle className="size-5 animate-spin" /> : rule.isEnabled ? <ToggleRight className="size-7 text-violet-600" /> : <ToggleLeft className="size-7" />}{rule.isEnabled ? "Activa" : "Pausada"}</button> : null}</div>)}
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <section className="surface overflow-hidden"><SettingsHeader icon={SlidersHorizontal} title="Campos personalizados" description="Información estructurada por tarea" action={canManage ? <button className="shrink-0 rounded-lg p-2 text-violet-600 hover:bg-violet-50" aria-label="Añadir campo personalizado"><Plus className="size-4" /></button> : undefined} /><div className="divide-y divide-slate-100 px-5 sm:px-6">{data.customFields.map((field) => <div key={field.id} className="flex items-center justify-between gap-3 py-3.5"><div className="min-w-0 flex-1"><p className="break-words text-xs font-bold text-slate-800">{field.name}</p><p className="mt-0.5 text-[11px] text-slate-400">{field.fieldType}</p></div>{field.isRequired ? <Badge color="#ef4444">Obligatorio</Badge> : <Badge>Opcional</Badge>}</div>)}{!data.customFields.length ? <p className="py-8 text-center text-xs text-slate-400">Sin campos personalizados.</p> : null}</div></section>
        <section className="surface overflow-hidden"><SettingsHeader icon={Eye} title="Vistas guardadas" description="Filtros personales y compartidos" action={<Badge>{data.savedViews.length}</Badge>} /><div className="divide-y divide-slate-100 px-5 sm:px-6">{data.savedViews.map((view) => <div key={view.id} className="flex items-center justify-between gap-3 py-3.5"><div className="min-w-0 flex-1"><p className="break-words text-xs font-bold text-slate-800">{view.name}</p><p className="mt-0.5 text-[11px] text-slate-400">{view.viewType}</p></div><Badge>{view.isShared ? "Compartida" : "Privada"}</Badge></div>)}{!data.savedViews.length ? <p className="py-8 text-center text-xs text-slate-400">Guarda filtros desde cualquier vista.</p> : null}</div></section>
      </div>

      <section className="surface overflow-hidden">
        <SettingsHeader icon={ShieldCheck} title="Seguridad de la cuenta" description={`Sesión activa hasta ${new Intl.DateTimeFormat("es-CO", { dateStyle: "medium", timeStyle: "short", timeZone }).format(new Date(sessionExpiresAt))}`} />
...
function SettingsHeader({ icon: Icon, title, description, action, stackActionOnMobile = false }: { icon: typeof Bot; title: string; description: string; action?: React.ReactNode; stackActionOnMobile?: boolean }) { return <div className={stackActionOnMobile ? "grid grid-cols-[auto_minmax(0,1fr)] items-start gap-x-3 border-b border-slate-100 px-5 py-4 sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:items-center sm:px-6" : "grid grid-cols-[auto_minmax(0,1fr)_auto] items-start gap-x-3 border-b border-slate-100 px-5 py-4 sm:items-center sm:px-6"}><span className="col-start-1 row-start-1 grid size-9 shrink-0 place-items-center rounded-xl bg-violet-50 text-violet-600"><Icon className="size-4" /></span><div className="col-start-2 row-start-1 min-w-0"><h2 className="break-words text-sm font-bold text-slate-900">{title}</h2><p className="mt-0.5 line-clamp-2 break-words text-[11px] leading-4 text-slate-400 sm:line-clamp-none">{description}</p></div>{action ? <div className={stackActionOnMobile ? "col-start-2 row-start-2 mt-3 justify-self-start sm:col-start-3 sm:row-start-1 sm:mt-0" : "col-start-3 row-start-1 shrink-0"}>{action}</div> : null}</div>; }
function SecurityFact({ icon: Icon, title, detail }: { icon: typeof Database; title: string; detail: string }) { return <div className="surface flex items-start gap-3 p-4"><span className="grid size-9 shrink-0 place-items-center rounded-xl bg-emerald-50 text-emerald-600"><Icon className="size-4" /></span><div className="min-w-0"><p className="break-words text-xs font-bold text-slate-800">{title}</p><p className="mt-0.5 break-words text-[11px] leading-4 text-slate-400">{detail}</p></div></div>; }
